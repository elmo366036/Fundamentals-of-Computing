...
def gen_sorted_sequences_verbose2(outcomes, length):
    """
    Function that creates all sorted sequences via gen_all_sequences
    """
    all_sequences = gen_all_sequences(outcomes, length)
    ...
    for sequence in all_sequences:
        ...
        sorted_sequence_tuple = tuple(sorted_list)
        sorted_sequences.add(sorted_sequence_tuple)

    return set(sorted_sequences)

def gen_sorted_sequences_verbose(outcomes, length):
    """
    Function that creates all sorted sequences via gen_all_sequences
    """
    all_sequences = gen_all_sequences(outcomes, length)
    ...
    #Verbose method
    sorted_sequences = set([])
    for combination in all_sequences:
        combination_list = list(combination)
        combination_list.sort()
        sorted_combination_tuple = tuple(combination_list)
        sorted_sequences.add(sorted_combination_tuple)
...
def run_example2():
    ...
    seq_outcomes = gen_sorted_sequences_verbose(outcomes, length)
    print "Computed", len(seq_outcomes), "sorted sequences of", str(length) ,"outcomes"
    print "Sequences were", seq_outcomes

    seq_outcomes = gen_sorted_sequences_verbose2(outcomes, length)
    print "Computed", len(seq_outcomes), "sorted sequences of", str(length) ,"outcomes"
    print "Sequences were", seq_outcomes

...
# http://www.codeskulptor.org/#user45_2rWp93zdYI82mMn_2.py
...
FRAME_BORDER = 50 				#this keeps the asteroid from spawning
                                ...
                                #lives and score
MAX_ROCKS = 6
MIN_ASTEROID_SHIP_SPAWN_DISTANCE = 100	#this keeps the asteriods from spawining
    ...
    for sprite in set(group):
        if sprite.collide(other_object):
            ...
            an_explosion = Sprite([sprite.get_position()[0],
                                  ...
                                  explosion_framerate_factor,
                                  explosion_sound)
            explosion_group.add(an_explosion)
            if other_object == my_ship:
                ship_explosion = Sprite([my_ship.get_position()[0],
                                         my_ship.get_position()[1]],
                                        [0, 0], 0, 0, explosion_image,
                                        explosion_info, explosion_dim,
                                        explosion_framerate_factor,
                                        explosion_sound)
...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated == False:
            ...
            current_image_index = (((time * math.fabs(self.framerate_factor)) % self.dim) // 1)
            if self.framerate_factor < 0:
                current_image_index = (self.dim - 1) - current_image_index
...
def rock_spawner():
    ...
    framerate_factor *= random.choice([-1, 1])
        #this should make framerate positive or negative
